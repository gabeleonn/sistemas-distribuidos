...
func HandleConsumeHotDeals(client *amqp.Client) error {
	queue, err := client.DeclareQueue("", true, false ,false)
	if err != nil {
		return err
	}

	if err := client.BindQueue(queue.Name, constants.EventPromotionFeatured); err != nil {
		return err
	}
	...
	for deliveries := range deliveries {
		var payload models.PromotionFeaturedPayload
		if err := parseHotDealDelivery(deliveries, &payload); err != nil {
			continue
		}

		payload.Promotion.HotDeal = true
		...
		logger.Get().Printf("promocao destacada: id=%s categoria=%s item=%s hotdeal=%t",
...
func parseHotDealDelivery(delivery amqp091.Delivery, payload *models.PromotionFeaturedPayload) error {
